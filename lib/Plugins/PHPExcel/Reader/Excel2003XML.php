...
/**
 * Plugins_PHPExcel_Reader_Excel2003XML
 *
 * @category   PHPExcel
 * @package    Plugins_PHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Plugins_PHPExcel_Reader_Excel2003XML extends Plugins_PHPExcel_Reader_Abstract implements Plugins_PHPExcel_Reader_IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new Plugins_PHPExcel_Reader_Excel2003XML
	 */
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	public function listWorksheetNames($pFilename)
	{
		...
		if (!$this->canRead($pFilename)) {
			throw new Plugins_PHPExcel_Reader_Exception($pFilename . " is an Invalid Spreadsheet file.");
		}

		$worksheetNames = array();

		$xml = simplexml_load_file($pFilename, 'SimpleXMLElement', Plugins_PHPExcel_Settings::getLibXmlLoaderOptions());
	...
	public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
	{
		...
		$underlineStyles = array (
				Plugins_PHPExcel_Style_Font::UNDERLINE_NONE,
				Plugins_PHPExcel_Style_Font::UNDERLINE_DOUBLE,
				Plugins_PHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING,
				Plugins_PHPExcel_Style_Font::UNDERLINE_SINGLE,
				Plugins_PHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING
			);
		$verticalAlignmentStyles = array (
				Plugins_PHPExcel_Style_Alignment::VERTICAL_BOTTOM,
				Plugins_PHPExcel_Style_Alignment::VERTICAL_TOP,
				Plugins_PHPExcel_Style_Alignment::VERTICAL_CENTER,
				Plugins_PHPExcel_Style_Alignment::VERTICAL_JUSTIFY
			);
		$horizontalAlignmentStyles = array (
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_GENERAL,
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_LEFT,
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_RIGHT,
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS,
				Plugins_PHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY
			);
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','Plugins_PHPExcel_Reader_Excel2003XML::_hex2str',$propertyName);
				$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_UNKNOWN;
				switch((string) $propertyAttributes) {
					case 'string' :
						$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
						$propertyValue = trim($propertyValue);
						break;
					case 'boolean' :
						$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
						$propertyValue = (bool) $propertyValue;
						break;
					case 'integer' :
						$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_INTEGER;
						$propertyValue = intval($propertyValue);
						break;
					case 'float' :
						$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
						$propertyValue = floatval($propertyValue);
						break;
					case 'dateTime.tz' :
						$propertyType = Plugins_PHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
...
//									echo $borderStyleKey.' = '.$borderStyleValue.'<br />';
									switch ($borderStyleKey) {
										case 'LineStyle' :
												$thisBorder['style'] = Plugins_PHPExcel_Style_Border::BORDER_MEDIUM;
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Index'])) {
						$columnID = Plugins_PHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']-1);
					}
...
//					echo '<b>Row '.$rowID.'</b><br />';
					...
					foreach($rowData->Cell as $cell) {
						...
						if (isset($cell_ss['Index'])) {
							$columnID = Plugins_PHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']-1);
						}
						...
						if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
							...
							if (isset($cell_ss['MergeAcross'])) {
								$columnTo = Plugins_PHPExcel_Cell::stringFromColumnIndex(Plugins_PHPExcel_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
							}
							$rowTo = $rowID;
							...
							if (isset($cellData_ss['Type'])) {
								...
								switch ($cellDataType) {
									...
									*/
									...
									case 'DateTime' :
											$type = Plugins_PHPExcel_Cell_DataType::TYPE_NUMERIC;
											$cellValue = Plugins_PHPExcel_Shared_Date::PHPToExcel(strtotime($cellValue));
											break;
									case 'Error' :
...
//								echo 'FORMULA<br />';
								...
								$columnNumber = Plugins_PHPExcel_Cell::columnIndexFromString($columnID);
									...
									foreach($temp as &$value) {
										...
										if ($key = !$key) {
											...
											foreach($cellReferences as $cellReference) {
												...
												$A1CellReference = Plugins_PHPExcel_Cell::stringFromColumnIndex($columnReference-1).$rowReference;
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
	...
	private static function _convertStringEncoding($string,$charset) {
		if ($charset != 'UTF-8') {
			return Plugins_PHPExcel_Shared_String::ConvertEncoding($string,'UTF-8',$charset);
		}
		return $string;
	}
