...
/**
 * Plugins_PHPExcel_Style_NumberFormat
 *
 * @category   PHPExcel
 * @package	Plugins_PHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Plugins_PHPExcel_Style_NumberFormat extends Plugins_PHPExcel_Style_Supervisor implements Plugins_PHPExcel_IComparable
{
	/* Pre-defined formats */
	 ...
	 * @var string
	 */
	protected $_formatCode	=	Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new Plugins_PHPExcel_Style_NumberFormat
	 *
	 ...
	 *		array(
	 *			'code' => Plugins_PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return Plugins_PHPExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$dateObj = Plugins_PHPExcel_Shared_Date::ExcelToPHPObject($value);
	...
	private static function _formatAsFraction(&$value, &$format)
	{
		...
		$GCD = Plugins_PHPExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
	 ...
	 */
	public static function toFormattedString($value = '0', $format = Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		if (($format === Plugins_PHPExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === Plugins_PHPExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return $value;
		}
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
										$value
										, strlen($right)
										, Plugins_PHPExcel_Shared_String::getDecimalSeparator()
										, Plugins_PHPExcel_Shared_String::getThousandsSeparator()
									);
