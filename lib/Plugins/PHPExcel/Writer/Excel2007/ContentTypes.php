...
/**
 * Plugins_PHPExcel_Writer_Excel2007_ContentTypes
 *
 * @category   PHPExcel
 * @package    Plugins_PHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Plugins_PHPExcel_Writer_Excel2007_ContentTypes extends Plugins_PHPExcel_Writer_Excel2007_WriterPart
{
	/**
	...
	public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = FALSE)
	{
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof Plugins_PHPExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof Plugins_PHPExcel_Worksheet_MemoryDrawing) {
	...
	{
		if (Plugins_PHPExcel_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new Plugins_PHPExcel_Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	Plugins_PHPExcel_Writer_Exception
	 */
	private function _writeDefaultContentType(Plugins_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	Plugins_PHPExcel_Writer_Exception
	 */
	private function _writeOverrideContentType(Plugins_PHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
