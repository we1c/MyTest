...
/**
 * Plugins_PHPExcel_Writer_Excel5_Workbook
 *
 * @category   PHPExcel
 * @package    Plugins_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Plugins_PHPExcel_Writer_Excel5_Workbook extends Plugins_PHPExcel_Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	...
	/**
	 * XF Writers
	 * @var Plugins_PHPExcel_Writer_Excel5_Xf[]
	 */
	 ...
	 * Fonts writers
	 *
	 * @var Plugins_PHPExcel_Writer_Excel5_Font[]
	 */
	 ...
	 *
	 * @param Plugins_PHPExcel_Style_Font $font
	 * @return int Index to FONT record
	 */
	public function _addFont(Plugins_PHPExcel_Style_Font $font)
	{
		...
		if(isset($this->_addedFonts[$fontHashCode])){
			...
			$fontWriter = new Plugins_PHPExcel_Writer_Excel5_Font($font);
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + strlen(Plugins_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = Plugins_PHPExcel_Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = Plugins_PHPExcel_Cell::coordinateFromString($printArea[0]);
				$printArea[1] = Plugins_PHPExcel_Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = Plugins_PHPExcel_Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = Plugins_PHPExcel_Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = Plugins_PHPExcel_Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = Plugins_PHPExcel_Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		if (count($this->_phpExcel->getNamedRanges()) > 0) {
			...
			foreach ($namedRanges as $namedRange) {
				...
				$range = Plugins_PHPExcel_Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . Plugins_PHPExcel_Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = Plugins_PHPExcel_Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = Plugins_PHPExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = Plugins_PHPExcel_Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = Plugins_PHPExcel_Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = Plugins_PHPExcel_Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = Plugins_PHPExcel_Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if(!empty($autoFilterRange)) {
				$rangeBounds = Plugins_PHPExcel_Cell::rangeBoundaries($autoFilterRange);
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = Plugins_PHPExcel_Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(Plugins_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
	 ...
	 *
	 * @param Plugins_PHPExcel_Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case Plugins_PHPExcel_Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case Plugins_PHPExcel_Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case Plugins_PHPExcel_Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		$data .= Plugins_PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		$numberFormatString = Plugins_PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (Plugins_PHPExcel_Shared_Date::getExcelCalendar() == Plugins_PHPExcel_Shared_Date::CALENDAR_MAC_1904) ?
