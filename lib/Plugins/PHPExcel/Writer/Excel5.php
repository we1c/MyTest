...
/**
 * Plugins_PHPExcel_Writer_Excel5
 *
 * @category   PHPExcel
 * @package    Plugins_PHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Plugins_PHPExcel_Writer_Excel5 extends Plugins_PHPExcel_Writer_Abstract implements Plugins_PHPExcel_Writer_IWriter
{
	/**
	 * PHPExcel object
	 *
	...
	/**
	 * Create a new Plugins_PHPExcel_Writer_Excel5
	 *
	...
	public function __construct(PHPExcel $phpExcel) {
		...
		$this->_parser		= new Plugins_PHPExcel_Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$saveDateReturnType = Plugins_PHPExcel_Calculation_Functions::getReturnDateType();
		...
		$this->_writerWorkbook = new Plugins_PHPExcel_Writer_Excel5_Workbook($this->_phpExcel,
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new Plugins_PHPExcel_Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
		...
		$OLE = new Plugins_PHPExcel_Shared_OLE_PPS_File(Plugins_PHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new Plugins_PHPExcel_Shared_OLE_PPS_File(Plugins_PHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new Plugins_PHPExcel_Shared_OLE_PPS_File(Plugins_PHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new Plugins_PHPExcel_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
		$res = $root->save($pFilename);

		Plugins_PHPExcel_Calculation_Functions::setReturnDateType($saveDateReturnType);
	 ...
	 * @return Plugins_PHPExcel_Writer_Excel5
	 */
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			$escher = new Plugins_PHPExcel_Shared_Escher();

			// dgContainer
			$dgContainer = new Plugins_PHPExcel_Shared_Escher_DgContainer();
			...
			// spgrContainer
			$spgrContainer = new Plugins_PHPExcel_Shared_Escher_DgContainer_SpgrContainer();
			$dgContainer->setSpgrContainer($spgrContainer);

			// add one shape which is the group shape
			$spContainer = new Plugins_PHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = Plugins_PHPExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = Plugins_PHPExcel_Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new Plugins_PHPExcel_Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = Plugins_PHPExcel_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = Plugins_PHPExcel_Cell::stringFromColumnIndex(Plugins_PHPExcel_Cell::stringFromColumnIndex($iInc - 1));
	...
	private function _buildWorkbookEscher()
	{
		...
		// dggContainer
		$dggContainer = new Plugins_PHPExcel_Shared_Escher_DggContainer();
		...
		// bstoreContainer
		$bstoreContainer = new Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof Plugins_PHPExcel_Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(Plugins_PHPExcel_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
					...
					$blip = new Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
					$blip->setData($blipData);

					$BSE = new Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE();
					$BSE->setBlipType($blipType);
					$BSE->setBlip($blip);

					$bstoreContainer->addBSE($BSE);

				} else if ($drawing instanceof Plugins_PHPExcel_Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case Plugins_PHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case Plugins_PHPExcel_Worksheet_MemoryDrawing::RENDERING_GIF:
					case Plugins_PHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG:
					case Plugins_PHPExcel_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = Plugins_PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeSummaryInformation(){
		...
		if($this->_phpExcel->getProperties()->getCreated()){
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   ...
								   'data'	=> array('data' => Plugins_PHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
