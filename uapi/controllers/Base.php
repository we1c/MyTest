...
class BaseController extends Yaf_Controller_Abstract
{
    ...
    public function init()
    {
        ...
        $key = "d11edd70fed958651b4619131ffbb236";
        ...
        if ( $token )
        {
            ...
            $this->respon( 0 , "Access Denied "  );
        }



        if ( !intval( $jid ) )  $this->respon( 0 , "Access Denied " );

        $key = $this->db->fetchOne("SELECT `key` FROM `channel` where id = " . $jid );

        if ( !$key ) $this->respon( 0 , "Access Denied " );

        $md5hash = md5( $key . $jid . $t);
        ...
        $res['jid'] = $jid;
        $res['log'] = json_encode( $this->getRequest() );
        $this->db->insert( 'channel_view_log' , $res);
